...
namespace BotX.Api.Executors
{
    internal sealed class MiddlewareExecutor
    {
        ...
        {
            // Добавляем SmartAppMiddleware, она отвечает за обработку запросов от SmartApp и вызов соответствующих зарегистрированных обработчиков
            MiddlewareExecutor.AddMiddleware(typeof(SmartAppMiddleware));

            // Добавляем SmartAppMiddleware, она отвечает за обработку запросов от system:internal_bot_notification и вызов соответствующих зарегистрированных обработчиков
            MiddlewareExecutor.AddMiddleware(typeof(InternalBotNotificationMiddleware));
