...
public class InternalBotNotificationMiddleware
{
    private const string SmartAppCommand = "system:internal_bot_notification";

    private readonly BotMiddlewareHandler next;
    private readonly ILogger<InternalBotNotificationMiddleware> _logger;
    private static Dictionary<string, MethodInfo> SmartAppMethods;

    static InternalBotNotificationMiddleware()
    {
        SmartAppMethods = new();

        var applicationAssembly = Assembly.GetEntryAssembly();
        var typesWithAttribute = applicationAssembly.GetExportedTypes()
            .Where(x => x.GetCustomAttribute(typeof(InternalBotControllerAttribute)) != null);

        foreach (var type in typesWithAttribute)
        {
            var methods = type.GetMethods()
                .Where(e => e.GetCustomAttributes(typeof(InternalControllerMethodAttribute))?.Any() == true).ToList();

            foreach (var method in methods)
            {
                var attrs = method.GetCustomAttributes<InternalControllerMethodAttribute>();
    ...
    public InternalBotNotificationMiddleware(BotMiddlewareHandler next, ILogger<InternalBotNotificationMiddleware> logger)
    {
        this.next = next;
        this._logger = logger;
    }
    ...
    public async Task InvokeAsync(UserMessage message, IBotMessageSender sender, IServiceProvider serviceProvider)
    {
            ...
            SmartAppMethods.Keys.Any(e => Regex.IsMatch(message.Command.Data.SmartAppData.Method, e)))
        {
            ...
            _logger.LogDebug($"InternalBotNotificationMiddleware InvokeAsync key - {key}; method - {method}");
        ...
        else
        {
            if (message.Command.Body == SmartAppCommand)
            {
                Console.WriteLine($"Received unregistered InternalBotNotificationMiddleware: \r\n {JsonConvert.SerializeObject(message.Command.Data.SmartAppData)}");
            }
            await next(message);
